...
def avg_lv_sim(model_words1, model_words2, threshold=0.7):
    ...
    for word1 in model_words1:
        for word2 in model_words2:
            ...
            if (non_sim := normalized_levenshtein(non_numeric1, non_numeric2)) >= 0.5 and numeric1 == numeric2:
...
def compute_tmwm_sim(title1, title2, alpha=0.602, beta=0.1, delta=0.4, threshold=0.5, epsilon=0):
    ...
    # Step 3: Compute average Levenshtein similarity for title words
    avg_lv_similarity = avg_lv_sim(set(title1.split()), set(title2.split()), threshold)
    ...
    for word1 in model_words1:
        for word2 in model_words2:
            ...
            # Check for conflicts or identical model words
