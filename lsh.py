...
def find_b_r(num_rows, t):
    ...
    for i in np.arange(0, 20):  # Iterate over i from 0 to 20 to allow for more values of b and r
        n = num_rows - i
        for r in range(2, 100):  # Iterate over r from 2 to 100
            if n % r == 0:  # Only consider cases where n is divisible by r
                b = n // r  # Compute b
                distance = abs((1 / b) ** (1 / r) - t)  # Calculate the distance to threshold

                if distance < min_distance:  # Update best values if they are closer to target
...
def apply_lsh(signature_matrix, r, n):
    ...
    for band in range(b):
        ...
        for product in range(cols):
            ...
            # Place product in unique bucket corresponding to the elements in the band
...
def evaluate_lsh_performance(true_duplicates, lsh_candidates):
    ...
    if len(lsh_candidates) == 0: print("no lsh candidates")
